...
void YOLOv8TRTPose::initConfig(std::string enginefile, float conf_thresholod) {
	RNG rng;
	for (int i = 0; i < 17; i++) {
	...
	this->conf_thresholod = conf_thresholod;
...
void YOLOv8TRTPose::detect(cv::Mat &frame, std::vector<DetectResult> &results) {
	...
	for (int i = 0; i < det_output.rows; i++) {
		...
		if (conf > this->conf_thresholod)
		{
			...
			cv::Mat pts = det_output.row(i).colRange(5, output_h);
			kypts.push_back(pts);
		}
	}
