...
// 初始化静态成员
std::shared_mutex InspSew::modelLoadMutex;
std::map<std::string, std::string> InspSew::sewDetectionModelMap;
std::map<std::string, std::string> InspSew::sewClassifyModelMap;
std::map<int, InspSewIn> InspSew::cameraConfigMap;

// 构造函数
InspSew::InspSew(std::string configPath, const cv::Mat& img, int cameraId, int jobId,
	...
	TXT(std::make_unique<TxtOperater>())
{
	...
	if (img.empty()) {
		outInfo.status.statusCode = SEW_RETURN_ALGO_ERR;
	...
	if (shouldLoadConfig)
	{
		...
		if (!rv_loadConfig) {
			outInfo.status.statusCode = SEW_RETURN_CONFIG_ERR;
	...
	if (outInfo.status.statusCode = SEW_RETURN_OK)
	{
...
// 验证摄像头ID对应的模型配置是否存在
bool InspSew::validateCameraModels(int cameraId) {
...
// 加载所有模型到ModelManager
bool InspSew::loadAllModels(InspSewOut& outInfo, bool ini) {
	if (!ini) {
...
// 读取参数的函数
bool InspSew::readParams(cv::Mat img, const std::string& filePath, InspSewIn& params, InspSewOut& outInfo, const std::string& fileName) {
	...
	while (!ifs.eof()) {
		...
		//是否存储中间图像(0:否  1:是)
		if (keyWord == "SEW_SAVE_DEBUG_IMAGE") {
			params.saveDebugImage = std::stoi(value);
		}
		else if (keyWord == "SEW_SAVE_RESULT_IMAGE") {
			params.saveResultImage = std::stoi(value);
		}
		else if (keyWord == "SEW_SAVE_LOG_TXT") {
			params.saveLogTxt = std::stoi(value);
		}
		else if (keyWord == "SEW_DRAW_RESULT") {
			params.drawResult = std::stoi(value);
		}
		else if (keyWord == "SEW_SAVE_TRAIN") {
			params.saveTrain = std::stoi(value);
		}
		else if (keyWord == "SEW_ROI_X") {
		...
		else if (keyWord == "SEW_HARDWARE_TYPE") {
			params.hardwareType = std::stoi(value);
		}
		else if (keyWord == "SEW_MODEL_TYPE") {
			params.modelType = std::stoi(value);
		}
		else if (keyWord == "SEW_LOCATE_MODEL_WEIGHTS_FLIE") {
			std::lock_guard<std::shared_mutex> lock(modelLoadMutex);
		...
		else if (keyWord == "SEW_LOCATE_MODEL_CONFIGURATIO_FLIE") {
		...
		else if (keyWord == "SEW_CLASSFY_MODEL_WEIGHTS_FLIE") {
...
void InspSew::Sew_SetROI(InspSewOut& outInfo) {
...
void InspSew::Sew_LocateSew(InspSewOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	if (outInfo.status.statusCode != SEW_RETURN_OK) {
	...
	for (int i = outInfo.locate.details.size() - 1; i >= 0; --i)
	{
		...
		else if (locate.className == "MID")	paramIndex = 2;
		...
		if (paramIndex != -1)
		{
			...
			if (paramIndex == 0)
			{
				if (locate.box.x < 5 && locate.box.width < outInfo.images.roiImg.cols*0.35)
				{
					valid = false;
				}
				else if (locate.box.x + locate.box.width > outInfo.images.roiImg.cols - 5 && locate.box.width < outInfo.images.roiImg.cols * 0.6 && outInfo.system.cameraId == 1)
				{
					valid = false;
				}
				else if (locate.box.x + locate.box.width > outInfo.images.roiImg.cols - 5 && locate.box.width < outInfo.images.roiImg.cols * 0.2 && outInfo.system.cameraId != 1)
				{
					valid = false;
				}
			}
	...
	{
		outInfo.status.errorMessage = "无目标!";
		Log::WriteAsyncLog("无目标!", ERR, outInfo.paths.logFile, true);
		outInfo.status.statusCode = SEW_RETURN_NO_TARGET;
	}
	...
	for (int i = 0; i < outInfo.locate.details.size(); i++)
	{
		...
		{
			outInfo.status.errorMessage = "缝线异常!";
			Log::WriteAsyncLog("缝线异常!", ERR, outInfo.paths.logFile, true);
			outInfo.status.statusCode = SEW_RETURN_BAD;
			break;
		}
	}
	...
	{
		if (outInfo.status.statusCode == SEW_RETURN_BAD)
		{
			outInfo.system.startTime = COM->time_t2string_with_ms();
			COM->CreateDir(outInfo.paths.trainDir + "LOCATE/BAD/");
			...
			saveJSONToFile(jsonData, outInfo.paths.trainDir + "LOCATE/BAD/" + outInfo.system.startTime + "_" + std::to_string(outInfo.system.cameraId) + "_" + std::to_string(outInfo.system.jobId) + ".json");
			cv::imwrite(outInfo.paths.trainDir + "LOCATE/BAD/" + outInfo.system.startTime + "_" + std::to_string(outInfo.system.cameraId) + "_" + std::to_string(outInfo.system.jobId) + ".jpg", m_img);

		}
		else if (outInfo.status.statusCode == SEW_RETURN_POSERR)
		{
			outInfo.system.startTime = COM->time_t2string_with_ms();
			COM->CreateDir(outInfo.paths.trainDir + "LOCATE/POSERR/");
			auto jsonData = generateXAnyLabelingJSON(
				outInfo.locate.details,
				outInfo.system.startTime + "_" + std::to_string(outInfo.system.cameraId) + "_" + std::to_string(outInfo.system.jobId) + ".jpg",
				m_img.rows,
			...
			saveJSONToFile(jsonData, outInfo.paths.trainDir + "LOCATE/POSERR/" + outInfo.system.startTime + "_" + std::to_string(outInfo.system.cameraId) + "_" + std::to_string(outInfo.system.jobId) + ".json");
			cv::imwrite(outInfo.paths.trainDir + "LOCATE/POSERR/" + outInfo.system.startTime + "_" + std::to_string(outInfo.system.cameraId) + "_" + std::to_string(outInfo.system.jobId) + ".jpg", m_img);

		}
		else if (outInfo.status.statusCode == SEW_RETURN_NO_TARGET)
		{
			outInfo.system.startTime = COM->time_t2string_with_ms();
...
void InspSew::Sew_CheckSewType(InspSewOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	if (outInfo.status.statusCode != SEW_RETURN_OK) {
...
void InspSew::Sew_DrawResult(InspSewOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	...
	for (int i = 0; i < outInfo.locate.details.size(); i++)
	{
		...
		else if (outInfo.locate.details[i].className == "MID")
		{
			rectangle(outInfo.images.outputImg, outInfo.locate.details[i].box, Colors::LIGHT_BLUE, 3, cv::LINE_AA);
			putTextZH(outInfo.images.outputImg,
				(outInfo.locate.details[i].className + "," + std::to_string(outInfo.locate.details[i].box.width) + "," + std::to_string(outInfo.locate.details[i].box.height) + "," + format(outInfo.locate.details[i].confidence)).c_str(),
				cv::Point(outInfo.locate.details[i].box.x, outInfo.locate.details[i].box.y + outInfo.locate.details[i].box.height + 10),
				Colors::LIGHT_BLUE, 45, FW_BOLD);
		}
	}
	...
	cv::Scalar statusColor = (outInfo.status.statusCode == SEW_RETURN_OK) ?
...
std::future<int> InspSew::RunInspectionAsync(InspSewOut& outInfo) {
	return std::async(std::launch::async, [this, &outInfo] {
		// 设置超时检查起点
		m_startTime = std::chrono::high_resolution_clock::now();

		// 执行主检测逻辑
		return Sew_Main(outInfo, true);
		});
}

int InspSew::Sew_Main(InspSewOut& outInfo, bool checkTimeout) {
	try {
		double time0 = static_cast<double>(cv::getTickCount());
		if (outInfo.status.statusCode == SEW_RETURN_OK)
		{
			Log::WriteAsyncLog("Sew_Main!", INFO, outInfo.paths.logFile, m_params.saveLogTxt);
			if (checkTimeout && CheckTimeout(m_params.timeOut)) {
				if (m_safeTimeoutFlag) {
					m_safeTimeoutFlag->store(true);
				}
				return SEW_RETURN_TIMEOUT;
			}
