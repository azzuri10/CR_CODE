...
// 初始化静态成员
std::shared_mutex InspHeatSeal::modelLoadMutex;
std::map<std::string, std::string> InspHeatSeal::sewDetectionModelMap;
std::map<std::string, std::string> InspHeatSeal::sewClassifyModelMap;
std::map<int, InspHeatSealIn> InspHeatSeal::cameraConfigMap;

// 构造函数
InspHeatSeal::InspHeatSeal(std::string configPath, const cv::Mat& img, int cameraId, int jobId,
	...
	TXT(std::make_unique<TxtOperater>())
{
	...
	if (img.empty()) {
		outInfo.status.statusCode = HEATSEAL_RETURN_ALGO_ERR;
	...
	if (shouldLoadConfig)
	{
		...
		if (!rv_loadConfig) {
			outInfo.status.statusCode = HEATSEAL_RETURN_CONFIG_ERR;
	...
	if (outInfo.status.statusCode = HEATSEAL_RETURN_OK)
	{
...
// 验证摄像头ID对应的模型配置是否存在
bool InspHeatSeal::validateCameraModels(int cameraId) {
...
// 加载所有模型到ModelManager
bool InspHeatSeal::loadAllModels(InspHeatSealOut& outInfo, bool ini) {
	if (!ini) {
	...
	try {
		...
		cv::Mat iniImg = cv::Mat::zeros(cv::Size(2500, 2000), CV_8UC3);
...
// 读取参数的函数
bool InspHeatSeal::readParams(cv::Mat img, const std::string& filePath, InspHeatSealIn& params, InspHeatSealOut& outInfo, const std::string& fileName) {
	...
	while (!ifs.eof()) {
		...
		//是否存储中间图像(0:否  1:是)
		if (keyWord == "HEATSEAL_SAVE_DEBUG_IMAGE") {
			params.saveDebugImage = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_SAVE_RESULT_IMAGE") {
			params.saveResultImage = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_SAVE_LOG_TXT") {
			params.saveLogTxt = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_DRAW_RESULT") {
			params.drawResult = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_SAVE_TRAIN") {
			params.saveTrain = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_ROI_X") {
		...
		else if (keyWord == "HEATSEAL_ROI_Y") {
		...
		else if (keyWord == "HEATSEAL_ROI_W") {
		...
		else if (keyWord == "HEATSEAL_ROI_H") {
		...
		else if (keyWord == "HEATSEAL_HARDWARE_TYPE") {
			params.hardwareType = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_MODEL_TYPE") {
			params.modelType = std::stoi(value);
		}
		else if (keyWord == "HEATSEAL_LOCATE_MODEL_WEIGHTS_FLIE") {
			std::lock_guard<std::shared_mutex> lock(modelLoadMutex);
		...
		else if (keyWord == "HEATSEAL_LOCATE_MODEL_CONFIGURATIO_FLIE") {
		...
		else if (keyWord == "HEATSEAL_CLASSFY_MODEL_WEIGHTS_FLIE") {
			...
			if (params.isCheckColor)
			{
				...
				{
					outInfo.status.errorMessage = "热合分类模型文件缺失!";
					Log::WriteAsyncLog(params.sewClassifyWeightsFile, ERR, outInfo.paths.logFile, true, "--热合分类模型文件缺失！");
					return false;
				}
			}
		}
...
void InspHeatSeal::HeatSeal_SetROI(InspHeatSealOut& outInfo) {
...
void InspHeatSeal::HeatSeal_LocateHeatSeal(InspHeatSealOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	if (outInfo.status.statusCode != HEATSEAL_RETURN_OK) {
	...
	if (outInfo.locate.details.empty())
	{
		...
		outInfo.status.statusCode = HEATSEAL_RETURN_NO_TARGET;
	}
	...
	for (int i = 0; i < outInfo.locate.details.size(); i++)
	{
		...
		{
			outInfo.status.errorMessage = "热合异常!";
			Log::WriteAsyncLog("热合异常!", ERR, outInfo.paths.logFile, true);
			outInfo.status.statusCode = HEATSEAL_RETURN_BAD;
			break;
		}
	}
	...
	{
		if (outInfo.status.statusCode == HEATSEAL_RETURN_BAD)
		{
		...
		else if (outInfo.status.statusCode == HEATSEAL_RETURN_NO_TARGET)
		{
			outInfo.system.startTime = COM->time_t2string_with_ms();
...
void InspHeatSeal::HeatSeal_CheckHeatSealType(InspHeatSealOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	if (outInfo.status.statusCode != HEATSEAL_RETURN_OK) {
...
void InspHeatSeal::HeatSeal_DrawResult(InspHeatSealOut& outInfo) {
	if (CheckTimeout(m_params.timeOut)) return;
	...
	cv::Scalar statusColor = (outInfo.status.statusCode == HEATSEAL_RETURN_OK) ?
...
std::future<int> InspHeatSeal::RunInspectionAsync(InspHeatSealOut& outInfo) {
	return std::async(std::launch::async, [this, &outInfo] {
		// 设置超时检查起点
		m_startTime = std::chrono::high_resolution_clock::now();

		// 执行主检测逻辑
		return HeatSeal_Main(outInfo, true);
		});
}

int InspHeatSeal::HeatSeal_Main(InspHeatSealOut& outInfo, bool checkTimeout) {
	try {
		double time0 = static_cast<double>(cv::getTickCount());
		if (outInfo.status.statusCode == HEATSEAL_RETURN_OK)
		{
			Log::WriteAsyncLog("HeatSeal_Main!", INFO, outInfo.paths.logFile, m_params.saveLogTxt);
			if (checkTimeout && CheckTimeout(m_params.timeOut)) {
				if (m_safeTimeoutFlag) {
					m_safeTimeoutFlag->store(true);
				}
				return HEATSEAL_RETURN_TIMEOUT;
			}
			...
			// 第2步:定位
			HeatSeal_LocateHeatSeal(outInfo);
			if (CheckTimeout(m_params.timeOut))
			{
				Log::WriteAsyncLog("超时!", INFO, outInfo.paths.logFile, m_params.saveLogTxt);
				return HEATSEAL_RETURN_TIMEOUT;
			...
			//// 第3步:分类
			//HeatSeal_CheckHeatSealType(outInfo);
			//if (CheckTimeout(m_params.timeOut))
			//{
			//	Log::WriteAsyncLog("超时!", INFO, outInfo.paths.logFile, m_params.saveLogTxt);
			//	return HEATSEAL_RETURN_TIMEOUT;
		...
		return HEATSEAL_RETURN_ALGO_ERR;
	}
