...
//
//            // label（兼容中文）
//            std::string label = s["label"].get<std::string>();
//
...
//
//// ===================== 主流程 =====================
//void processFolder(const std::string& root, int cameraId) {
//    std::string okDir = root + "/OK/";
//    std::string ngDir = root + "/NG/";
...
//
//            InspCode* pInspCode = new InspCode(ProjectConstants::CONFIG_PATH, img, cameraId, jobId, true, 1000000, outInfo);
...
//
//            // ================= 转换检测结果 =================
//            std::vector<FinsObject> detObjects;
//            for (auto& line : outInfo.locate.lineDetails) {
//                for (auto& obj : line) {
...
//
//            for (auto& detObj : detObjects) {
//                bool matched = false;
//                for (auto& [gtRect, gtLabel] : gtObjects) {
//                    if (detObj.className == gtLabel && IoU(detObj.box, gtRect) > 0.75) {
//                        matched = true;
//                        break;
//                    }
//                }
...
//// ===================== 主入口 =====================
//int main() {
//    std::string rootFolder = "F:/TRAIN_SAMPLE_3.0/CODE/PET/YYDDQ/20250606";  // 修改成你数据集的根目录
//    int cameraId = 0; // 相机编号
//
//    processFolder(rootFolder, cameraId);
//
//    return 0;
//}
