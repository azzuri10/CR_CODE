...
private:
    ...
    static std::map<int, InspBoxBagIn> cameraConfigMap;

public:
    InspBoxBag(std::string configPath, const cv::Mat& img, int cameraId, int jobId,
    ...
    int BoxBag_Main(InspBoxBagOut& outInfo, bool checkTimeout = false);
    std::future<int> RunInspectionAsync(InspBoxBagOut& outInfo);
...
private:
    ...
    bool readParams(cv::Mat img, const std::string& filePath, InspBoxBagIn& params,
        InspBoxBagOut& outInfo, const std::string& fileName);
    bool validateCameraModels(int cameraId);
    bool loadAllModels(InspBoxBagOut& outInfo, bool ini);

    void BoxBag_SetROI(InspBoxBagOut& outInfo);
    void BoxBag_LocateBoxBag(InspBoxBagOut& outInfo);
    void BoxBag_CheckBoxBagType(InspBoxBagOut& outInfo);
    void BoxBag_DrawResult(InspBoxBagOut& outInfo);
};
